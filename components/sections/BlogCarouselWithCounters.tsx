...
export function BlogCarouselWithCounters() {
  ...
  const counters = [
    { labelKey: "customers", value: 1200 },
    { labelKey: "orders", value: 3500 },
    { labelKey: "projects", value: 150 },
    { labelKey: "cities_served", value: 25 },
  ];
            ...
            {blogs.map((blog, idx) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/70 to-transparent p-6 text-white">
                  <h3 className="text-2xl font-bold mb-2">{t(blog.titleKey)}</h3>
                  <p className="text-base">{t(blog.descKey)}</p>
                </div>
              </div>
            ))}
            {/* Timer Bar */}
